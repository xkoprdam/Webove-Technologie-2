{{-- resources/views/scheduled-mails/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Scheduled Mail') }}
        </h2>
    </x-slot>

    <div class="p-4 max-w-2xl mx-auto space-y-6">
        <form method="POST"
              action="{{ route('scheduled-mails.update', $scheduledMail) }}"
              class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="template_id" name="template_id"
                    ...
                    @foreach($templates as $tpl)
                        <option value="{{ $tpl->id }}"
                                {{ old('template_id', $scheduledMail->template_id)==$tpl->id?'selected':'' }}>
                            {{ $tpl->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="recipients" name="recipients[]" multiple
                    ...
                    @foreach($contacts as $c)
                        <option value="{{ $c->id }}"
                                {{ in_array($c->id, old('recipients', $scheduledMail->recipients))?'selected':'' }}>
                            {{ $c->name }} ({{ $c->email }})
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <x-text-input id="send_at" name="send_at" type="datetime-local"
                              :value="old('send_at', $scheduledMail->send_at->format('Y-m-d\TH:i'))"
