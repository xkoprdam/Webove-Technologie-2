    ...
    <div class="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div class="flex items-center space-x-4">
            ...
            if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
                echo '<a href="docs.php" class="text-gray-300 hover:text-white">Dokumentácia</a>';
...
<div class="max-w-7xl mx-auto mt-6 px-4">
    ...
    <?php
    if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
        echo '<h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">Vitaj ' . htmlspecialchars($_SESSION['fullname']) . '</h2><br>';
    } else {
        echo '<h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">Pre úpravu tabuľky sa prosím prihláste.</h2>';
    }
    ?>
    <br>
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex space-x-4">
            <?php if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) { ?>
                ...
                <button id="uploadJsonBtn" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Nahrať JSON</button>
            <?php } ?>
        </div>
        <div class="flex flex-col items-end">
            ...
            <select id="pageLength" class="w-32 p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
...
<div id="addModal" class="modal fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center hidden">
    <div class="modal-content bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-4xl">
...
<!-- Upload JSON Modal -->
<div id="uploadJsonModal" class="modal fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center hidden">
    <div class="modal-content bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Nahrať laureátov cez JSON</h2>
        <form id="uploadLaureatesForm" enctype="multipart/form-data">
            <div class="mb-4">
                <label for="jsonFile" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Vyberte JSON súbor:</label>
                <input type="file" id="jsonFile" name="jsonFile" accept=".json" class="mt-1 w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500">
            </div>
            <div class="flex justify-end space-x-2">
                <button type="button" id="closeUploadJsonModal" class="px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-md hover:bg-gray-400 dark:hover:bg-gray-500">Zrušiť</button>
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Nahrať</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        const uploadJsonModal = $('#uploadJsonModal');
        const uploadJsonBtn = $('#uploadJsonBtn');
        const closeUploadJsonModalBtn = $('#closeUploadJsonModal');

        uploadJsonBtn.on('click', function() {
            $('#jsonFile').val(''); // Reset file input
            uploadJsonModal.removeClass('hidden');
        });

        closeUploadJsonModalBtn.on('click', function() {
            uploadJsonModal.addClass('hidden');
        });

        $(window).on('click', function(event) {
            if (event.target === uploadJsonModal[0]) {
                uploadJsonModal.addClass('hidden');
            }
        });
